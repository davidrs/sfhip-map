...
crimeStatsDistrict <- crimeJoined %>% group_by(Category, PdDistrict) %>% summarize(total_crimes = n())

# Standardize column names
colnames(crimeStatsDistrict) <- c("category", "police_district", "total_crimes")

# Filter to desired crime type, list of possible options is in possibleCrimes DF
filteredCrimeDistrict <- crimeStatsDistrict[crimeStatsDistrict$category %in% desiredCrimes, ]
